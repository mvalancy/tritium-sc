...
class TestWarAnnouncerInit:
    def test_default_state(self, bus: EventBus) -> None:
        ...
        assert ann.kill_count == 0
        assert ann.wave_kill_count == 0
...
class TestKillAnnouncements:
    def test_target_eliminated(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        ...
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
        assert len(ann_msgs) == 1
        assert ann_msgs[0]["data"]["category"] == "kill"
    ...
    def test_kill_increments_counts(self, announcer: WarAnnouncer) -> None:
        assert announcer.kill_count == 0
        assert announcer.wave_kill_count == 0
        announcer._on_target_eliminated({"killer_name": "A", "target_name": "B"})
        assert announcer.kill_count == 1
        assert announcer.wave_kill_count == 1
        announcer._on_target_eliminated({"killer_name": "A", "target_name": "C"})
        assert announcer.kill_count == 2
        assert announcer.wave_kill_count == 2
...
class TestKillStreaks:
    def test_streak_3(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        q = bus.subscribe()
        announcer._on_kill_streak({"streak": 3, "killer_name": "Rover Alpha"})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
        assert len(ann_msgs) == 1
        assert ann_msgs[0]["data"]["category"] == "kill_streak"
    ...
    def test_streak_5(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        q = bus.subscribe()
        announcer._on_kill_streak({"streak": 5, "killer_name": "Drone Beta"})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
    ...
    def test_streak_7(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        q = bus.subscribe()
        announcer._on_kill_streak({"streak": 7, "killer_name": "Turret C"})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
    ...
    def test_streak_10_godlike(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        q = bus.subscribe()
        announcer._on_kill_streak({"streak": 10, "killer_name": "MegaUnit"})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
    ...
    def test_streak_unknown_count(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        ...
        q = bus.subscribe()
        announcer._on_kill_streak({"streak": 15, "killer_name": "Hero"})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
        assert len(ann_msgs) == 1
        assert "15 KILL STREAK" in ann_msgs[0]["data"]["text"]
    ...
    def test_streak_interrupts_kill_on_debounce(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        ...
        announcer._on_kill_streak({"streak": 5, "killer_name": "A"})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
        ...
        assert "kill" in categories
        assert "kill_streak" in categories
...
class TestWaveAnnouncements:
    ...
    def test_wave_start_resets_wave_kills(self, announcer: WarAnnouncer) -> None:
        announcer._kill_count = 5
        announcer._wave_kill_count = 5
        announcer._on_wave_start({"wave_number": 2, "wave_name": "Test", "hostile_count": 4})
        assert announcer.wave_kill_count == 0
        assert announcer.kill_count == 5  # total unchanged

    def test_wave_complete(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        q = bus.subscribe()
        announcer._on_wave_complete({"wave_number": 2, "kills": 8})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
...
class TestGameOver:
    def test_victory(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        q = bus.subscribe()
        announcer._on_game_over({"result": "victory", "total_kills": 42, "waves_completed": 10})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
    ...
    def test_defeat(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        q = bus.subscribe()
        announcer._on_game_over({"result": "defeat", "total_kills": 15, "waves_completed": 3})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
    ...
    def test_defeat_is_default(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        q = bus.subscribe()
        announcer._on_game_over({"total_kills": 5, "waves_completed": 1})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
    ...
    def test_victory_bypasses_debounce(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        ...
        q = bus.subscribe()
        announcer._on_target_eliminated({"killer_name": "A", "target_name": "B"})
        announcer._on_game_over({"result": "victory", "total_kills": 1, "waves_completed": 1})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
...
class TestDebounce:
    def test_rapid_kills_debounced(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        ...
        q = bus.subscribe()
        for i in range(5):
            announcer._on_target_eliminated({"killer_name": "A", "target_name": f"T{i}"})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
...
class TestReset:
    def test_reset_clears_counters(self, announcer: WarAnnouncer) -> None:
        announcer._kill_count = 10
        announcer._wave_kill_count = 5
        announcer._last_announcement = 99.0
        announcer._last_by_category["kill"] = "some phrase"
        announcer.reset()
        assert announcer.kill_count == 0
        assert announcer.wave_kill_count == 0
...
class TestThinkingWarMode:
    def test_war_mode_context_format(self) -> None:
        from amy.thinking import WAR_MODE_CONTEXT
...
class TestPriorityOrdering:
    ...
    def test_high_priority_preempts_during_debounce(self, announcer: WarAnnouncer, bus: EventBus) -> None:
        ...
        announcer._on_kill_streak({"streak": 10, "killer_name": "Hero"})
        msgs = []
        while not q.empty():
            msgs.append(q.get_nowait())
        ann_msgs = [m for m in msgs if m["type"] == "amy_announcement"]
...
class TestPhraseBanks:
    ...
    def test_all_kill_phrases_have_placeholders(self) -> None:
        for phrase in KILL_PHRASES:
            assert "{killer}" in phrase or "{target}" in phrase
