...
class TestCombatSystemTick:
    ...
    def test_projectile_eliminates_target(self):
        ...
        assert event["target_id"] == "h1"
        assert event["killer_id"] == "t1"
        assert event["killer_name"] == "Turret"
...
class TestCombatSystemKillStreaks:
    def test_kill_streak_at_3(self):
        bus = SimpleEventBus()
        combat = CombatSystem(bus)
        streak_sub = bus.subscribe("kill_streak")
        ...
        # Should have KILLING SPREE event
        event = streak_sub.get(timeout=1.0)
        assert event["streak"] == 3
        assert event["streak_name"] == "KILLING SPREE"
        assert event["killer_id"] == "t1"

    def test_kill_streak_at_5(self):
        bus = SimpleEventBus()
        combat = CombatSystem(bus)
        streak_sub = bus.subscribe("kill_streak")
        ...
        # Drain streak_sub: should get KILLING SPREE at 3, RAMPAGE at 5
        events = []
        while not streak_sub.empty():
            events.append(streak_sub.get_nowait())
        names = [e["streak_name"] for e in events]
        assert "KILLING SPREE" in names
        assert "RAMPAGE" in names

    def test_reset_streaks(self):
        bus = SimpleEventBus()
        combat = CombatSystem(bus)
        combat._kill_streaks["t1"] = 5
        combat.reset_streaks()
        assert len(combat._kill_streaks) == 0

    def test_reset_single_streak(self):
        bus = SimpleEventBus()
        combat = CombatSystem(bus)
        combat._kill_streaks["t1"] = 5
        combat._kill_streaks["t2"] = 3
        combat.reset_streak("t1")
        assert "t1" not in combat._kill_streaks
        assert combat._kill_streaks["t2"] == 3
...
class TestCombatSystemStreakName:
    def test_streak_milestones(self):
        assert CombatSystem._get_streak_name(3) == "KILLING SPREE"
